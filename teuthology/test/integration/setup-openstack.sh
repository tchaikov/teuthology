...
#        debian-7.0: debian-wheezy-7.1
    entercloudsuite:
      ...
      images:
        ...
        centos-7.0: GNU/Linux CentOS 7 x64 RAW
...
function setup_paddles() {
	...
	sudo -u postgres psql -c "CREATE USER paddles with PASSWORD 'paddles';"
    ...
    (
	    ...
	    -e "s|^job_log_href_templ = 'http://qa-proxy.ceph.com/teuthology|job_log_href_templ = 'http://$ip|" \
	    -e "/sqlite/d" \
...
function setup_bashrc() {
    if test -f ~/.bashrc && grep -qq 'openrc.sh' ~/.bashrc ; then
        echo "OK openrc.sh found in ~/.bashrc"
    else
...
EOF
        echo "ADDED openrc.sh and openstack command to ~/.bashrc"
    fi
}
