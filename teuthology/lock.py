...
def lock_many_openstack(ctx, num, machine_type, user=None, description=None,
    ...
    if hasattr(ctx, 'openstack_cluster') and ctx.openstack_cluster:
        machines = provision.OpenStack(ctx.openstack_cluster).create(
            num, os_type, os_version, arch, resources_hint=None)
    else:
